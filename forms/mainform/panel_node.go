...
import (
	...
	"github.com/gazercloud/gazernode/forms/charts"
	"github.com/gazercloud/gazernode/forms/cloud"
	"github.com/gazercloud/gazernode/forms/maps"
...
type PanelNode struct {
	...
	panelUnits          *units.PanelUnits
	panelCloud          *cloud.PanelCloud
	panelPublicChannels *public_channels.PanelPublicChannels
	panelCharts         *charts.PanelCharts
	panelMaps           *maps.PanelMaps
	panelUsers          *users.PanelUsers
	...
	c.client.OnSessionOpen = func() {
		...
		/*if MainFormInstance.currentNodeWidget == &c {
			c.client.ListOfUnits(func(items []nodeinterface.UnitListResponseItem, err error) {
				if len(items) == 0 {
					dialog := tools.NewFormWizard(&c, c.client)
					dialog.ShowDialog()
...
func (c *PanelNode) OnInit() {
	...
	c.panelUnits = units.NewPanelUnits(panelContent, c.client)
	panelContent.AddWidgetOnGrid(c.panelUnits, 0, 0)
	c.panelCloud = cloud.NewPanelCloud(panelContent, c.client)
	panelContent.AddWidgetOnGrid(c.panelCloud, 0, 1)
	c.panelPublicChannels = public_channels.NewPanelPublicChannels(panelContent, c.client)
	panelContent.AddWidgetOnGrid(c.panelPublicChannels, 0, 2)
	c.panelCharts = charts.NewPanelCharts(panelContent, c.client)
	panelContent.AddWidgetOnGrid(c.panelCharts, 0, 3)
	c.panelMaps = maps.NewPanelMaps(panelContent, c.client)
	...
	c.btnSettings = c.panelBottom.AddButtonOnGrid(20, 0, "Settings", func(event *uievents.Event) {
		dialog := tools.NewServiceDialog(c.panelBottom, c.client)
		dialog.ShowDialog()
	...
	c.lblNodeName.OnClick = func(ev *uievents.Event) {
		dialog := tools.NewNodeNameDialog(c, c.client, c.lastNodeName)
		dialog.OnAccept = func() {
			c.updateNodeName()
		}
		dialog.ShowDialog()
