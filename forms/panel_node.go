...
func (c *PanelNode) OnInit() {
	...
	c.panelConnection = c.AddPanelOnGrid(0, 1)
	c.panelConnection.AddTextBlockOnGrid(0, 0, "Connection")
	c.panelConnection.AddButtonOnGrid(1, 0, "Connection", func(event *uievents.Event) {
		dialog := NewNodeConnectionDialog(c)
		dialog.OnAccept = func() {
			c.client.SessionOpen(dialog.Connection.UserName, dialog.Connection.Password, func(response nodeinterface.SessionOpenResponse, err error) {
				if err == nil {
					var conn local_user_storage.NodeConnection
					conn.UserName = dialog.Connection.UserName
					conn.Address = dialog.Connection.Address
					conn.SessionToken = response.SessionToken
	...
	// Bottom
	c.panelBottom = c.AddPanelOnGrid(0, 2)
...
func (c *PanelNode) timerUpdate() {
	...
	c.client.GetStatistics(func(statistics common_interfaces.Statistics, err error) {
		if c.lblStatistics != nil {
			if err == nil {
				c.lblStatistics.SetForeColor(c.InactiveColor())
